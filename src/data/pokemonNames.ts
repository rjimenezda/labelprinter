/**
 * National Dex id -> species name, index 0 = id 1 (bulbasaur). Bundled as
 * a static asset rather than fetched live so the RENDERER never depends
 * on PokeAPI at print/view time -- only the editor calls the API (once,
 * when a Pokemon is picked), and bakes the resolved id into the doc. The
 * artwork URL is deterministic from the id alone (see pokemon.ts), so
 * this table is the only other thing the render path needs, and it's
 * fully offline.
 *
 * Source: https://pokeapi.co/api/v2/pokemon-species?limit=2000 (fetched
 * 2026-09-16, National Dex 1-1025).
 */
